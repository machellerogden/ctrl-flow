...
function Readers({ genSym = GenSym(), resolver = v => nil } = {}) {
    ...
    async function shorthand(args, generateNewName) {
        ...
        if (typeof args === 'string' && args.startsWith('$')) return params(args);
        const resolved = await resolve(args);
        if (resolved.StartAt) return submachine(args, resolved);
        if ([ 'string', 'function' ].includes(typeof resolved)) return task({ Resource: resolved });
        if (Array.isArray(resolved)) {
            if (resolved.length === 1) return task({ Resource: resolved[0] });
            if (resolved.length === 2) {
                if (typeof resolved[0] === 'string' && resolved[0].startsWith('$')) return task({ InputPath: resolved[0], Resource: resolved[1] });
                if (typeof resolved[1] === 'string' && resolved[1].startsWith('$')) return task({ Resource: resolved[0], ResultPath: resolved[1] });
                if (typeof resolved[0] !== 'string') return task({ Parameters: resolved[0], Resource: resolved[1] });
                if (typeof resolved[1] !== 'string') return task({ Resource: resolved[0], Result: resolved[1] });
                return task({ Parameters: resolved[0], Resource: resolved[1] });
            }
            if (resolved.length === 3) {
                if (typeof resolved[0] === 'string' && resolved[0].startsWith('$')) {
                    if (typeof resolved[2] === 'string' && resolved[2].startsWith('$')) return task({ InputPath: resolved[0], Resource: resolved[1], ResultPath: resolved[2] });
                    return task({ InputPath: resolved[0], Resource: resolved[1], Result: resolved[2] });
                }
                if (typeof resolved[2] === 'string' && resolved[2].startsWith('$')) {
                    return task({ Parameters: resolved[0], Resource: resolved[1], ResultPath: resolved[2] });
                }
                return task({ Parameters: resolved[0], Resource: resolved[1], Result: resolved[2] });
            }
        }
