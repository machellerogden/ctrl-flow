...
function Readers({ genSym = GenSym(), resolver = v => nil } = {}) {
    ...
    function getName(resource, generateNewName) {
        ...
        if (typeof curr !== 'string') throw new Error('Invalid input');
        if (generateNewName) return genSym(curr.split(':').pop());
        if (curr.includes(':')) return curr;
        if (nameMap[curr]) return nameMap[curr];
        const Name = curr.split(':').pop();
        nameMap[curr] = Name;
        return Name;
    }
    ...
    const isJSONPath = v =>
        typeof v === 'string' && (v === '$' || v.startsWith('$.') || v.startsWith('$['));
    ...
    async function shorthand(args, generateNewName) {
        ...
        if ([ 'string', 'function' ].includes(typeof resolved))
            return task({ Resource: resolved, Parameters: [] });
        ...
        if (resolved.length === 2) {
            ...
            if (isJSONPath(resolved[1]) || typeof resolved[1] !== 'string') {
