...
function Readers({ genSym }) {
    ...
    function _from([ Name, InputPath ] = []) {
        const state = shorthand(Name);
        return { ...state, InputPath };
    }

    function to([ Name, OutputPath ] = []) {
        const state = shorthand(Name);
        return { ...state, OutputPath };
    }

    function io([ Name, InputPath = '$', OutputPath = '$' ] = []) {
        const state = shorthand(Name);
        return { ...state, InputPath, OutputPath };
    }
