...
function Readers({ genSym = GenSym(), resolver = v => v } = {}) {
    const nameMap = {};
    ...
    async function resolve(arg) {
        ...
        return (await resolver(args)) || args;
    }

    async function shorthand(args, generateNewName) {
        const task = Task(generateNewName);
        args = await resolve(args);
        return !Array.isArray(args)    ? task({ Resource: args })
             : args.length === 1       ? task({ Resource: args[0] })
             : isObject(args[0])       ? task({ Parameters: args[0], Resource: args[1], ResultPath: args[2] })
             : args[0].startsWith('$') ? task({ InputPath: args[0], Resource: args[1], ResultPath: args[2] })
             : /* default */             task({ Resource: args[0], ResultPath: args[1] });
    }
